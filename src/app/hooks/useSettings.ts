...
// Hook for the CURRENT user's own settings (editable)
export function useSettings(userId: number) {
  ...
  const updateSetting = useCallback(async (name: keyof UserSettings, value: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/settings.php`, {
        ...
        body: JSON.stringify({ user_id: userId, setting_name: name, setting_value: value }),
      });
...
// One-shot fetch of another user's public settings (not a hook, just a utility)
